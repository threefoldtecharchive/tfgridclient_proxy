...
func (g *RetryingGridProxyClient) Contracts(filter ContractFilter, pagination Limit) (res []Contract, err error) {
	f := func() error {
		res, err = g.cl.Contracts(filter, pagination)
		return err
	}
	backoff.RetryNotify(f, bf(g.timeout), notify("contracts"))
	return
}
